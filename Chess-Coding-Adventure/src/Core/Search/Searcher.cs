	...
	public class Searcher
	{
		...
		void RunIterativeDeepeningSearch()
		{
			for (int searchDepth = 1; searchDepth <= 256; searchDepth++)
			{
				...
				System.Int64 nps = elapsedMs > 0 ? searchDiagnostics.numPositionsEvaluated * 1000 / elapsedMs : 0;

				Console.Write("time {0} nodes {1} nps {2} pv {3}\n",
					elapsedMs,
					searchDiagnostics.numPositionsEvaluated,
					nps, MoveUtility.GetMoveNameUCI(hasSearchedAtLeastOneMove?bestMoveThisIteration:bestMove));
